...
// START STUDENTS---------------------------------------------------------------------------------------------------------
    ...
    public void showStudents(String search) throws SQLException {
        ...
        if(search.equals("")){
            query =
                "SELECT l_name,f_name from student join enrolled using (stu_id) where sec_id = 'activeSecId'";
        }else{
            query =
                    "SELECT * from student join enrolled using (stu_id) where sec_id = 'activeSecId' WHERE f_name ILIKE ('%' ||'\" + search  + \"'|| '%') OR l_name ILIKE ('%' ||'\" + search  + \"'|| '%')username ILIKE ('%' ||'" + search  + "'|| '%') ORDER BY l_name";
        }
        try (PreparedStatement stmt = db.prepareStatement(query)) {
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
    ...
    @Command
    public void grade(String itemName, String userName) throws SQLException {
        ...
        String queryCheck =
                ...
                "where sec_id='"+activeSecId+"' and title='"+itemName+"'"+
                "order by(type)";
        try (PreparedStatement stmt = db.prepareStatement(queryCheck)){
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
        ...
        if (gradeExists){
            String updateQuery = "Update grade set recieved = 'score' where student_id = (select student_id from student where username = 'username')";
            try (PreparedStatement stmt = db.prepareStatement(updateQuery)){
                stmt.executeUpdate();
            }
        } else {
            String insertQuery = "Insert into grade (stu_id,a_id,recieved) values ((Select student_id from student where username = 'userName'),(Select a_id from assignment where title = 'itemName'),score)";
            try (PreparedStatement stmt = db.prepareStatement(insertQuery)){
                stmt.executeUpdate();
            }
        }
